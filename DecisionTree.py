...
def print_tree(node, spacing=""):
    ...
    if isinstance(node, Leaf):
        ...
        print(spacing+"nodeID" + str(node.id))
        return
...
def getLeafNodes(node, leafNodes=[]):
    ...
    if node is not None:
        # if len(node.false_branch) == 0 and len(node.true_branch) == 0:
        if isinstance(node,Leaf):
...
def computeAccuracy(rows, node):
    ...
    numAccurate = 0
    for row in rows:
        predLabel = classify(row, node)
        if predLabel == row[-1]:
            numAccurate += 1

    return round(numAccurate / totalRows, 2)

def pruningStrategy (node, listofNode=[]):

    rootNode = prune_tree(node, listofNode)
    accuracy = computeAccuracy(node.rows, rootNode)